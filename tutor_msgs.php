...
if ($session && $user->getUserType() === "tutor") {
        ...
        <div class="container bg-white bordered-dark pad-bottom-lg">
            ...
            <div>
                ...
                <div class="">
                    <section id="msg-display" class="col-md-8 col-sm-8 bordered-dark " style="min-height: 200px">
                        ...
                        <div class="message-box ">
                            ...
                            <div class="message-body" style="min-height: 200px;max-height: 900px; overflow: auto;">
                                ...
                                <div class="col-md-10 bordered-dark pad-sm" style="max-height: 490px; overflow: auto;">
                                    <?php
                                            ...
                                            if (substr($val{'filetype'}, 0, 5) === "image") {
                                                if (file_exists($val{'dir'})) {
                                                    echo "<div class='col-sm-6' >
                                                        ...
                                                        <a href='" . ($val{'dir'}) . "' download='" . ($val{'filename'}) . "'><i class='fa fa-download'></i></a>Filename: " . $val{'filename'} . "&nbsp;</div>";
                                                    echo "";
                                                    if ($i % 2 === 0) {
                                                ...
                                                echo "'>&nbsp;</i>" . $val{'filename'} . "&nbsp;<a class='bordered btn btn-xs' title='download file' href='" . ($val{'dir'}) . "' download='" . ($val{'filename'}) . "'>&nbsp;<i class='fa fa-download'>&nbsp;</i></a></span>";
                                            }
                                        }
                                ...
                                if (count($uploads)) {
                                    echo "<hr/>";
                                    echo "<p class='bordered pad-sm'>{$i} files uploaded</p>";
                                }
                                ?>
                            </div>
                            <hr>
                            <div class="message-footer pad-xs margin-bottom-sm">
                                ...
                                if (isset ($get_msg{0}{'level'}) && $get_msg{0}{'level'} === 'affirm' && null !== $task_affirmed && $is_dept) {
                                    ...
                                    echo "<div class=\"pull-right text-success bordered pad-xs clearfix'\">
										<i class='fa fa-check-circle'>&nbsp;</i>You just accepted a project!
										</div>
                                ...
                                <?php if (null !== $msg_detail) {
                                    echo "<ul class=\"menu_user_wrap link-list \">
                                    <li class=\"margin-bottom-sm\">
                                        <div class=\"label bg_tint_dark bg-purple-light not-rounded pad-sm\">" . $user->getDept() . "</div>
                                    </li>";
                                    ...
                                    <?php if (!$is_dept && ($get_msg{0}{'level'}) === 'affirm') {
                                        ?>
                                        <div class="alert alert-warning not-rounded custom-shadow pad-lg " role="alert">
                                            Please wait for the concerned student to make payment first before
                                            proceeding
                                            with the project:
                                            "<?php echo ucfirst(substr($msg_detail{'projtname'}, 9)) ?>".
                    ...
                    <hr class="visible-xs">
                    <section id="sidebar" class="col-md-4 col-sm-4 no-pad pad-left-xs">
                        <div class="bordered-dark pad-xs  pad-bottom-lg">
                            ...
                            echo ((count($all_dept_msgs) - count($seen_dept_msgs)) > 0) ? (count($all_dept_msgs) - count($seen_dept_msgs)) : '';
                            echo "</span>";

                            echo "<div class=\"list-group margin-top-xs fadeIn " . (($is_dept) ? "in" : '') . " collapse\" id='dept_msgs'>";

                            foreach ($all_dept_msgs as $msgs) {
                                echo "<div class=\"list-group-item no-pad pad-xs not-rounded " . (($is_dept && $get_msg{0}{'mid'} === $msgs{'mid'}) ? " active-border" : '') . "\">";
                                echo "<a title='Click to view' href='" . $user->getUserType() . "_msgs.php?msg_id=" . $msgs{'mid'} . md5("dept_msg") . "'  class='" . $msgs{'level'} . " '><i class='" . ((!in_array($msgs{'mid'}, $seen_dept_msgs)) ? "fa fa-envelope-o" : "fa fa-envelope-open-o") . "' >&nbsp;</i>" . ucfirst(shorten($msgs{'topic'}, 15)) . "</a></div>";
                            }
                            echo "</div>";
                            ...
                            echo "<a style='text-decoration: none; cursor:pointer' data-toggle='collapse' href='#personal_msgs' class='label bg-purple-light bg_tint_dark pad-sm not-rounded' title='click to toggle' onclick=\"if(this.childNodes.item(1).classList.contains('fa-minus-circle')){
                                             ...
                                             this.childNodes.item(1).classList.remove('fa-plus-circle');
                                             this.childNodes.item(1).classList.add('fa-minus-circle');
                                         }\">Personal:&nbsp;&nbsp;<span class=\"label bg-green bordered bg-warning bg_tint_light  \">" . count($all_personal_msgs) . "</span> <i class='fa fa-" . ((!$is_dept) ? "minus" : 'plus') . "-circle'>&nbsp;</i></a>";
                            echo "<span class=\"label  bg-purple-light \" style='margin-top:-10px; margin-left: -1em; position:absolute'>";
                            echo ((count($all_personal_msgs) - count($seen_personal_msgs)) > 0) ? (count($all_personal_msgs) - count($seen_personal_msgs)) : '';
                            echo "</span>";
