...
class Student extends User
{
    ...
    public function askQuestion($topic, $dept, $msg, $upload = [], $timenum = 1, $period, $urgent = (1 | 0), $type, $proj)
    {
        ...
        $query = "INSERT INTO questions (qid, topic, message, projtname, urgent, by_who, type, status, starts, expires, match_tutor, ask_date,  answer_date) VALUES (NULL, '$topic', '$msg', '$upload', '$urgent', '" . $this->_id . "', '$type', 'pending', NULL,'$expires' , ,, )";
...
(NULL,'$qid', '" . $this->getDept() . "', '$duration', '$level', NOW())";
    ...
    {
        $query = "SELECT qid, topic, message, projtname, urgent, by_who, type, status, starts,  to_date(cast(expires AS TEXT), 'MM/DD/YYYY') AS expires  , match_tutor, to_date(cast(ask_date AS TEXT), 'MM/DD/YYYY') AS ask_date, answer_date FROM questions WHERE (by_who='" . $this->getID() . "' AND type='assignment')";
        return $result = $this->_conn->querySQLi($query, [])->fetchAll(2);
    }

    public function getAllProjects()
    {
        $query = "SELECT qid, topic, message, projtname, urgent, by_who, type, status, starts,  to_date(cast(expires AS TEXT), 'MM/DD/YYYY') AS expires  , match_tutor, to_date(cast(ask_date AS TEXT), 'MM/DD/YYYY') AS ask_date, answer_date FROM questions WHERE by_who='" . $this->_id . "' AND type='project'";
        return $result = $this->_conn->querySQLi($query, [])->fetchAll(2);
    }
