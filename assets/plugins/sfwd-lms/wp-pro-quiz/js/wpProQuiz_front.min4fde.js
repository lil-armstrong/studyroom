(function($) {
	...
	$.wpProQuizFront = function(element, options) {
		...
		var checker = function(name, data, $question, $questionList) {
			...
			var version = 0;
			var isDiffversion = $.isArray(data.version);
			...
			var func = {
				...
				matrix_sort_answer: function() {
					...
					$items.each(function() {
						...
						if($stringItem.length && $.inArray(String(i), String($stringItem.data('pos')).split(',')) >= 0) {
//						if(i == $stringItem.data('pos')) {
				...
				free_answer: function() {
					...
					if($.inArray($.trim(value).toLowerCase(), data.coordinates) >= 0) {
				...
				cloze_answer: function() {
					$questionList.find('.wpProQuiz_cloze').each(function(i, v) {
						...
						if($.inArray(inputText, data.coordinates[i]) >= 0) {
							if(isDiffversion) {
								version += data.version[i];
							}
			...
			if(!isDiffversion && coordinates) {
				version = data.version;
			}
			
			return {c: coordinates, p: version, s: statistcAnswerData};
		}; 
...
//				$e.find('.wpProQuiz_cloze input').val('');
			...
			loadQuizDataAjax: function(quizStart) {
				plugin.methode.ajax({
					...
					console.log($e.find('.wpProQuiz_sending'));
